...
%feed forward (Part 1)
basic = ((Y'*log(h)) + ((1-Y)'*(log(1-h))));
J = (-1/m)*sum(diag(basic));
...
regularize = (lambda/(2*m))*(sum(sum((the1.^2))) + sum(sum((the2.^2))));
J += regularize;
...
for t = 1:m
  ...
  y_t = Y(t,:);
  del_3 = a3 .- y_t;
  gd_z2 = sigmoidGradient(z2);
  del_2 = (del_3*(Theta2(:,2:end))).*(gd_z2);
